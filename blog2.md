...
To begin covering the plumbing for what was shown in Part 1, we will begin with the maven `pom.xml`. As discussed in the
 [Spring Boot OAuth Documentation][1], we will need the basics for a spring web app:
...
`serviceaccounts.openshift.io/oauth-redirectreference.primary`. To explain this annotation, first let's take a look at
...
not have a static URL to input here as the Route host is parametrized and can be generated by OpenShift in the case the
user leaves the parameter (`HOSTNAME_HTTP`) blank:
...
`system:serviceaccounts:oauth-example:spring-boot-oauth-app`. Then for the `clientSecret`, the Service Account's
secret token is used. One can retrieve this via the `oc serviceaccounts get-token` command:
...
So basically, the above implementation traverses this object though `metadata.name` to get `developer` as the Principal
object.
...
- Setting up an OpenShift Service Account with a OAuth Redirect URI annotation
